...
# some basic functions (mostly fundamental shapes for fitting etc)
...
# functions for forward modeling of pxtd images
def pxtd_inst(t, t0=0, dist = 5, Td3hep = 5, Tdtn = 5, Tddn = 5, Yd3hep = 1000, Ydtn = 1000, Yddn = 1000):
    # pass this function the distance, temperature, and counts and it will give you an approvimate time trace for each species
...
def reaction_tof_matrix(reaction, time, distance, energy):
    v = td.get_pop_velocities(reaction, energy) 
    T = np.array(time)
    Tc = T = T[0]
    g = np.zeros_like(T)
    TOF = distance/v
